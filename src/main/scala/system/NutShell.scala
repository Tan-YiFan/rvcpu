...
class riscv_cpu(implicit val p: NutCoreConfig) extends Module with HasSoCParameter {
  ...
  val xbar_io = if (p.FPGAPlatform) {
    Module(new SimpleBusCrossbarNto1(2))
  } else {
    Module(new SimpleBusCrossbarNto1(1))
  }
  ...
  xbar_io.io.in(0) <> memAddrMap.io.out
  io.mem <> xbar_io.io.out.toAXI4(true)
  ...
  if (p.FPGAPlatform) { xbar_io.io.in(1) <> extDev }
  else { io.mmio.getOrElse(null) <> extDev }
